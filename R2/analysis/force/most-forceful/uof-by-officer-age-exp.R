...
ordered.age.buckets <- c('25 or younger', '26 - 30', '31 - 35', '36 - 40', '41 - 45', '46 - 50', '51 or older')
...
uof.bucketed <- uof.for.year %>% mutate(
  ...
  exp.bucket = sapply(as.integer(Officer.years.exp.at.time.of.UOF), exp.bucket.function)
)
...
age.buckets <- age.buckets %>% mutate(
  count = sapply(age.buckets$age.bucket, function(age.bucket) {
    sum(adp.for.year$age.bucket == age.bucket)
  }),
...
gen.plotly.json(p.uof.by.officer.age.exp, "force-by-officer-age-exp")
